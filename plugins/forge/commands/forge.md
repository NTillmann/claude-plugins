...
```
On return, post e.g.: `📋 Plan: converged in {planRounds} round(s){, or note if it hit the cap / aborted}. Starting build.` The build workflow creates or loads the companion checklist, validates it before implementation, and reports any genuine blocker; there is no checklist file to validate here for plans without a pre-existing companion.
...
- **On plan done:** record `result.planRounds/planConverged` **and `result.scenarios`** (the detected-scenario array). If the notification status was `failed` (threw) → §Q4 retry/fail. Otherwise post `📋 [I.id] Plan: …`, then **launch build** passing `scenarios: I.result.scenarios` (set `phaseReached=build`, new `taskId`), persist, stop. The build workflow creates or loads and validates the companion checklist before implementation; do not try to validate a checklist between plan and build because it may not exist yet.
